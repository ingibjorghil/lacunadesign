	...
	protected function extended_vat_check_id() {
		...
		if ( get_option( 'woocommerce_eu_vat_store_id', '' ) != '' ) {
			...
			if (is_array($response) && !empty( $response['body'] ) && $response['body'] == "true" ) {
				...
				$info = sprintf(__('The store VAT ID (%s) could not be confirmed as valid, so VAT number validity checks will fail.', 'wc_eu_vat_compliance'), $storevat_country.' '.$storevat_id);

				if (!empty($this->vatnumber_class->fault_code)) {
					$info .= ' '.__('Error:', 'wc_eu_vat_compliance').' '.$this->vatnumber_class->fault_code_to_text();
				}
